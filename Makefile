...
client-debug-consume: build-debug
	docker compose run --rm -it --remove-orphans rst ./target/debug/jrpk client --address=jrpk:1133 --topic=posts --partition=0 --file=test.json --max-frame-size=1m consume --from=1000 --until=2000 --batch-bytes-size=1MiB --max-wait-ms=100

client-debug-produce: build-debug
	docker compose run --rm -it --remove-orphans rst ./target/debug/jrpk client --address=jrpk:1133 --topic=posts --partition=0 --file=test.json --max-frame-size=1m produce --batch-rec-count=1000
